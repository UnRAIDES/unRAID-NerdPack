                ...
                for pack in source_packages:
                    if package in pack:
                        ...
                        if update:
                            ...
                            if updatePackage(pack):
                                print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
                                os.makedirs(os.path.join("./","backup"), exist_ok=True)
                                shutil.move(os.path.join(dir_path,path), os.path.join("./","backup",path))
...
def updatePackage(package):
    ...
    filename = wget.download(f"https://slackware.uk/slackware/slackware64-15.0/slackware64/ap/{package}", out=dir_path)
